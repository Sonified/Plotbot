#!/usr/bin/env python3
"""
Simpler test - just check if br * bt works without lambda
"""
...
print("Creating b_product WITHOUT lambda...")
b_product = plotbot.custom_variable('b_product', plotbot.mag_rtn_4sa.br * plotbot.mag_rtn_4sa.bt)
...
print(f"b_product sample: {b_product.data[:5] if len(b_product.data) >= 5 else b_product.data}")
...
if b_product.data.shape == expected.shape:
    if np.allclose(b_product.data, expected, rtol=1e-9, equal_nan=True):
        print("\n✅ SUCCESS! br * bt works WITHOUT lambda!")
    else:
