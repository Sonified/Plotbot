...
Generated on: 2025-07-23T12:15:45.707207
...
class psp_waves_test_class:
    ...
    def __init__(self, imported_data):
        ...
        if imported_data is None:
            ...
            print_manager.dependency_management("Calculating psp_waves_test variables...")
            self.calculate_variables(imported_data)
            self.set_ploptions()
            print_manager.status("Successfully calculated psp_waves_test variables.")
        
        # Auto-register with data_cubby (following plotbot pattern)
        from plotbot.data_cubby import data_cubby
        data_cubby.stash(self, class_name='psp_waves_test')
        print_manager.dependency_management("Registered psp_waves_test with data_cubby")
    ...
    def __getattr__(self, name):
        ...
        print_manager.dependency_management('psp_waves_test getattr helper!')
    ...
    def __setattr__(self, name, value):
        ...
        if name in allowed_attrs or name in self.raw_data:
            ...
            print_manager.dependency_management('psp_waves_test setattr helper!')
            print(f"'{name}' is not a recognized attribute, friend!")
            available_attrs = list(self.raw_data.keys()) if self.raw_data else []
            print(f"Try one of these: {', '.join(available_attrs)}")
...
print_manager.dependency_management('initialized psp_waves_test class')
