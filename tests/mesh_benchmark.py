...
class ComponentStorageApproach:
    ...
    def get_times_mesh(self) -> np.ndarray:
        """Reconstruct mesh on demand with caching"""
        if self._times_mesh is None:
            start_time = time.perf_counter()
            self._times_mesh = np.meshgrid(
                self.datetime_array,
                np.arange(len(self.pitch_angles)),
                indexing='ij'
            )[0]
...
class FullMeshStorageApproach:
    """Alternative: Store complete mesh directly"""
    
    def __init__(self, datetime_array: np.ndarray, pitch_angles: np.ndarray):
        # Pre-compute and store the full mesh
        start_time = time.perf_counter()
        ...
        # Store components for reference (in real scenario, might not need these)
        self.datetime_array = datetime_array
        self.pitch_angles = pitch_angles
...
def create_realistic_data(time_points: int, energy_channels: int) -> Tuple[np.ndarray, np.ndarray]:
    """Create realistic electron/proton data dimensions"""
    ...
    # Energy channels/pitch angles - typical EPAD has ~32 pitch angle bins
    pitch_angles = np.linspace(0, 180, energy_channels)
    
    return datetime_array, pitch_angles

def benchmark_scenario(time_points: int, energy_channels: int, iterations: int = 100) -> BenchmarkResults:
    """Run comprehensive benchmark for a specific data size scenario"""
    
    print(f"\n🔬 Benchmarking: {time_points:,} time points × {energy_channels} channels")
    print(f"   Data volume: ~{(time_points * energy_channels * 8 / 1024**2):.1f} MB per mesh")
    
    # Create test data
    datetime_array, pitch_angles = create_realistic_data(time_points, energy_channels)
...
def run_comprehensive_benchmark():
    ...
    # Realistic PSP data scenarios
    scenarios = [
        # (time_points, energy_channels, description)
        (1000, 32, "Short interval, standard resolution"),
        (10000, 32, "~7 hours, standard resolution"), 
        (50000, 32, "~35 hours, standard resolution"),
        (100000, 32, "~70 hours, standard resolution"),
        (10000, 64, "~7 hours, high resolution"),
        (50000, 64, "~35 hours, high resolution"),
    ]
    ...
    for time_points, channels, description in scenarios:
        scenario_key = f"{time_points}x{channels}"
        results[scenario_key] = {
            'description': description,
            'data': benchmark_scenario(time_points, channels, iterations=50)
        }
