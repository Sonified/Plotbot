...
from .data_classes.psp_mag_classes import mag_rtn_4sa_class, mag_rtn_4sa, mag_rtn_class, mag_rtn, mag_sc_4sa_class, mag_sc_4sa, mag_sc_class, mag_sc
from .data_classes.psp_electron_classes import epad_strahl_class, epad, epad_strahl_high_res_class, epad_hr
from .data_classes.psp_proton_classes import proton_class, proton, proton_hr_class, proton_hr
...
def get_data(trange: List[str], *variables):
    ...
    *variables : object
    ...
    for var in variables:
        ...
        # === NEW: Check for specific class instances first ===
        if isinstance(var, mag_rtn_4sa_class):
            data_type = 'mag_RTN_4sa'
        elif isinstance(var, mag_rtn_class):
            data_type = 'mag_RTN' # Corrected key
        elif isinstance(var, mag_sc_4sa_class):
            data_type = 'mag_SC_4sa' # Corrected key
        elif isinstance(var, mag_sc_class):
            data_type = 'mag_SC' # Corrected key
        elif isinstance(var, epad_strahl_class):
            data_type = 'spe_sf0_pad' # Corrected key
        elif isinstance(var, epad_strahl_high_res_class):
             data_type = 'spe_af0_pad' # Corrected key
        elif isinstance(var, proton_class):
             data_type = 'spi_sf00_l3_mom' # Corrected key
        elif isinstance(var, proton_hr_class):
             data_type = 'spi_af00_L3_mom' # Corrected key
        # Now check for FITS and HAM instances
        elif isinstance(var, proton_fits_class):
            data_type = 'proton_fits'
            subclass_name = getattr(var, 'subclass_name', '?') # Keep subclass logic here
        elif isinstance(var, ham_class):
            data_type = 'ham'
            subclass_name = getattr(var, 'subclass_name', '?') # Keep subclass logic here
        # === END NEW Instance Check ===

        # Original checks (now as fallbacks or for specific component types)
        # elif type(var).__name__ in ('module', 'type'): # Check for module/type (less common)
        #     try:
        #         dt_name = var.__name__
        #         # Ensure it's a known type and not a local CSV source (like sf00/sf01 itself)
        #         if dt_name in data_types and data_types[dt_name].get('file_source') != 'local_csv':
        #             data_type = dt_name
        #         # else: Ignore sf00/sf01 passed directly, handled by proton_fits instance check
        #     except (AttributeError, TypeError):
        #         pass # data_type remains None
        elif hasattr(var, 'data_type') and hasattr(var, 'plot_options'): # More robust check for plot_manager components
            dt = var.data_type
            # Ensure it's not already handled by instance checks above
            if data_type is None and dt != 'proton_fits' and dt != 'ham' and data_types.get(dt, {}).get('file_source') != 'local_csv':
                data_type = dt
                subclass_name = getattr(var, 'subclass_name', '?')
            # elif data_type is not None: # Already identified by instance, just grab subclass if needed
            #     if subclass_name is None: # Only update if not already set by instance check
            #         subclass_name = getattr(var, 'subclass_name', '?')
        # elif is_proton_fits_var: # Original FITS check (redundant with isinstance)
        #     data_type = 'proton_fits'
        #     subclass_name = getattr(var, 'subclass_name', '?')
        # elif is_ham_var: # Original HAM check (redundant with isinstance)
        #     data_type = 'ham'
        #     subclass_name = getattr(var, 'subclass_name', '?')

        # Process if a data type was determined
        if data_type:
            print_manager.debug(f"  Identified data_type: {data_type} (Subclass: {subclass_name or 'N/A'})") # Added debug
    ...
    # ===> ADDED DEBUG PRINT <===
    print_manager.debug(f"DEBUG_GET_DATA: About to loop through required_data_types: {required_data_types}")
    ...
    for data_type in required_data_types:
        ...
        print_manager.debug(f"DEBUG_GET_DATA: Condition check for {data_type}: needs_import={needs_import}, needs_refresh={needs_refresh}")
        ...
        if needs_import or needs_refresh:
            ...
            if hasattr(target_instance, 'update'):
                try:
                    ...
                    print_manager.debug(f"DEBUG_GET_DATA: Before calling update, target_instance.source_filenames = {getattr(target_instance, 'source_filenames', 'AttributeMissing')}")
                    ...
                    # ===> ADDED CHECK HERE <===
                    if not getattr(target_instance, 'source_filenames', None):
                        print_manager.warning(f"CRITICAL_DEBUG: target_instance.source_filenames is MISSING/EMPTY immediately after update!")
