...
Generated on: 2025-07-21T16:48:16.902374
...
class psp_waves_auto_class:
    ...
    def __init__(self, imported_data):
        ...
        object.__setattr__(self, 'class_name', 'psp_waves_auto')
        object.__setattr__(self, 'data_type', 'psp_waves_auto')
        ...
        object.__setattr__(self, '_original_cdf_file_path', 'docs/implementation_plans/CDF_Integration/KP_wavefiles/PSP_wavePower_2021-04-29_v1.3.cdf')

        if imported_data is None:
            self.set_ploptions()
            print_manager.dependency_management("No data provided; initialized with empty attributes.")
        else:
            print_manager.dependency_management("Calculating psp_waves_auto variables...")
            self.calculate_variables(imported_data)
            self.set_ploptions()
            print_manager.status("Successfully calculated psp_waves_auto variables.")
        
        # Auto-register with data_cubby (following plotbot pattern)
        from plotbot.data_cubby import data_cubby
        data_cubby.stash(self, class_name='psp_waves_auto')
        print_manager.dependency_management("Registered psp_waves_auto with data_cubby")
    ...
    def __getattr__(self, name):
        ...
        print_manager.dependency_management('psp_waves_auto getattr helper!')
    ...
    def __setattr__(self, name, value):
        ...
        if name in allowed_attrs or name in self.raw_data:
            ...
            print_manager.dependency_management('psp_waves_auto setattr helper!')
            print(f"'{name}' is not a recognized attribute, friend!")
            available_attrs = list(self.raw_data.keys()) if self.raw_data else []
            print(f"Try one of these: {', '.join(available_attrs)}")
    ...
    def set_ploptions(self):
        ...
        print_manager.dependency_management("Setting up plot options for psp_waves_auto variables")
        

        self.wavePower_LH = plot_manager(
            self.raw_data['wavePower_LH'],
            plot_options=ploptions(
                data_type='psp_waves_auto',
...
psp_waves_auto = psp_waves_auto_class(None)
print_manager.dependency_management('initialized psp_waves_auto class')
