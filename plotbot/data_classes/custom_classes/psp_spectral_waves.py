...
Source: docs/implementation_plans/CDF_Integration/KP_wavefiles/PSP_WaveAnalysis_2021-04-29_0600_v1.2.cdf
...
class psp_spectral_waves_class:
    """
    ...
    def __init__(self, imported_data):
        ...
        object.__setattr__(self, 'class_name', 'psp_spectral_waves')
        object.__setattr__(self, 'data_type', 'psp_spectral_waves')
        ...
        # Store original CDF file path for data loading
        object.__setattr__(self, '_original_cdf_file_path', 'docs/implementation_plans/CDF_Integration/KP_wavefiles/PSP_WaveAnalysis_2021-04-29_0600_v1.2.cdf')

        if imported_data is None:
            self.set_ploptions()
            print_manager.dependency_management("No data provided; initialized with empty attributes.")
        else:
            print_manager.dependency_management("Calculating psp_spectral_waves variables...")
            self.calculate_variables(imported_data)
            self.set_ploptions()
            print_manager.status("Successfully calculated psp_spectral_waves variables.")
        
        # Auto-register with data_cubby (following plotbot pattern)
        from plotbot.data_cubby import data_cubby
        data_cubby.stash(self, class_name='psp_spectral_waves')
        print_manager.dependency_management("Registered psp_spectral_waves with data_cubby")
    ...
    def __getattr__(self, name):
        ...
        print_manager.dependency_management('psp_spectral_waves getattr helper!')
    ...
    def __setattr__(self, name, value):
        ...
        if name in allowed_attrs or name in self.raw_data:
            ...
            print_manager.dependency_management('psp_spectral_waves setattr helper!')
            print(f"'{name}' is not a recognized attribute, friend!")
            available_attrs = list(self.raw_data.keys()) if self.raw_data else []
            print(f"Try one of these: {', '.join(available_attrs)}")
    ...
    def set_ploptions(self):
        ...
        self.FFT_time_1 = plot_manager(
            ...
            plot_options=ploptions(
                data_type='psp_spectral_waves',
                var_name='FFT_time_1',
                class_name='psp_spectral_waves',
...
psp_spectral_waves = psp_spectral_waves_class(None)
print_manager.dependency_management('initialized psp_spectral_waves class')
