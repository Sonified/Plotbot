...
Generated on: 2026-01-07T13:16:32.677189
Source: /Users/robertalexander/GitHub/Plotbot/data/cdf_files/PSP_WaveAnalysis_2021-04-29_0600_v1.2.cdf
...
class psp_waves_spectral_class:
    """
    ...
    def __init__(self, imported_data):
        ...
        object.__setattr__(self, 'class_name', 'psp_waves_spectral')
        object.__setattr__(self, 'data_type', 'psp_waves_spectral')
        ...
        object.__setattr__(self, '_original_cdf_file_path', '/Users/robertalexander/GitHub/Plotbot/data/cdf_files/PSP_WaveAnalysis_2021-04-29_0600_v1.2.cdf')
    ...
    def get_subclass(self, subclass_name):
        ...
        print_manager.dependency_management(f"[PSP_WAVES_SPECTRAL_CLASS_GET_SUBCLASS] Attempting to get subclass/property: {subclass_name} for instance ID: {id(self)}")
        ...
        if hasattr(self, subclass_name):
            ...
            if not subclass_name.startswith('_'): 
                ...
                print_manager.dependency_management(f"[PSP_WAVES_SPECTRAL_CLASS_GET_SUBCLASS] Found '{subclass_name}' as a direct attribute/property. Type: {type(retrieved_attr)}")
                return retrieved_attr
            else:
                print_manager.dependency_management(f"[PSP_WAVES_SPECTRAL_CLASS_GET_SUBCLASS] '{subclass_name}' is an internal attribute, not returning via get_subclass.")
        ...
        if hasattr(self, 'raw_data') and self.raw_data and subclass_name in self.raw_data.keys():
            ...
            print_manager.dependency_management(f"[PSP_WAVES_SPECTRAL_CLASS_GET_SUBCLASS] Found '{subclass_name}' as a key in raw_data. Type: {type(component)}. This might be raw data.")
            return component

        # If not found as a direct attribute or in raw_data keys
        print_manager.warning(f"[PSP_WAVES_SPECTRAL_CLASS_GET_SUBCLASS] '{subclass_name}' is not a recognized subclass, property, or raw_data key for instance ID: {id(self)}.")
        ...
        print_manager.dependency_management(f"[PSP_WAVES_SPECTRAL_CLASS_GET_SUBCLASS] Available properties/attributes: {available_attrs}")
        print_manager.dependency_management(f"[PSP_WAVES_SPECTRAL_CLASS_GET_SUBCLASS] Available raw_data keys: {available_raw_keys}")
        return None
