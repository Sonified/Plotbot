...
class demo_wave_power_class:
    ...
    def __init__(self, imported_data):
        ...
        object.__setattr__(self, 'class_name', 'demo_wave_power')
        object.__setattr__(self, 'data_type', 'demo_wave_power')
        ...
        if imported_data is None:
            ...
            print_manager.dependency_management("Calculating demo_wave_power variables...")
            self.calculate_variables(imported_data)
            self.set_plot_config()
            print_manager.status("Successfully calculated demo_wave_power variables.")
        
        # Auto-register with data_cubby (following plotbot pattern)
        from plotbot.data_cubby import data_cubby
        data_cubby.stash(self, class_name='demo_wave_power')
        print_manager.dependency_management("Registered demo_wave_power with data_cubby")
    ...
    def __getattr__(self, name):
        ...
        print_manager.dependency_management('demo_wave_power getattr helper!')
    ...
    def __setattr__(self, name, value):
        ...
        if name in allowed_attrs or name in self.raw_data:
            ...
            print_manager.dependency_management('demo_wave_power setattr helper!')
            print(f"'{name}' is not a recognized attribute, friend!")
            available_attrs = list(self.raw_data.keys()) if self.raw_data else []
            print(f"Try one of these: {', '.join(available_attrs)}")
    ...
    def set_plot_config(self):
        ...
        print_manager.dependency_management("Setting up plot options for demo_wave_power variables")
        

        self.wavePower_LH = plot_manager(
            self.raw_data['wavePower_LH'],
            plot_config=plot_config(
                data_type='demo_wave_power',
                var_name='wavePower_LH',
                class_name='demo_wave_power',
...
demo_wave_power = demo_wave_power_class(None)
print_manager.dependency_management('initialized demo_wave_power class')
