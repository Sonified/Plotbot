...
Generated on: 2025-07-21T13:30:19.566087
...
class test_quick_class:
    ...
    def __init__(self, imported_data):
        ...
        object.__setattr__(self, 'class_name', 'test_quick')
        object.__setattr__(self, 'data_type', 'test_quick')
        ...
        if imported_data is None:
            ...
            print_manager.dependency_management("Calculating test_quick variables...")
            self.calculate_variables(imported_data)
            self.set_plot_config()
            print_manager.status("Successfully calculated test_quick variables.")
    ...
    def __getattr__(self, name):
        ...
        print_manager.dependency_management('test_quick getattr helper!')
    ...
    def __setattr__(self, name, value):
        ...
        if name in allowed_attrs or name in self.raw_data:
            ...
            print_manager.dependency_management('test_quick setattr helper!')
            print(f"'{name}' is not a recognized attribute, friend!")
            available_attrs = list(self.raw_data.keys()) if self.raw_data else []
            print(f"Try one of these: {', '.join(available_attrs)}")
    ...
    def set_plot_config(self):
        ...
        print_manager.dependency_management("Setting up plot options for test_quick variables")
        

        self.wavePower_LH = plot_manager(
            self.raw_data['wavePower_LH'],
            plot_config=plot_config(
                data_type='test_quick',
                var_name='wavePower_LH',
                class_name='test_quick',
...
test_quick = test_quick_class(None)
print_manager.dependency_management('initialized test_quick class')
