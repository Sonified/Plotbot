...
2.  **Add Interpolation Helper:** Implement the private `_interpolate_to_self_time(self, source_time, source_data)` method using `scipy.interpolate.interp1d` with `kind='linear'`, `bounds_error=False`, and `fill_value=np.nan`.
3.  **Implement `@property vsw_mach`:** Create the property, including cache check, dependency fetch (`spi_sf00_l3_mom`), checking for `vp` or calculating magnitude from `V_rtn`, calling the interpolation helper, performing the Mach number calculation (`vp / valfven`), caching the result, and error handling.
