# Implementation Plan: Degrees from Perihelion Axis (v2 - Fixed)

**Date:** 2025-05-07

**Status: Implementation Complete & Verified**

- Implementation of `use_degrees_from_perihelion` mode in `multiplot.py` is complete.
- Initial debugging identified and fixed a timestamp unit mismatch in `XAxisPositionalDataMapper.map_to_position` affecting interpolation accuracy.
- Subsequent debugging identified that `multiplot.py` was incorrectly using a stale perihelion time reference for all panels due to flawed logic in the original `plotbot/utils.py:get_perihelion_time` function.
- The `get_perihelion_time` function in `utils.py` has been rewritten to correctly find the *closest* perihelion event based on the input time, resolving the axis offset issue.
- The `PERIHELION_TIMES` dictionary in `utils.py` was updated to include encounters up to E23 (as per available specific timestamp data).
- The outdated `get_encounter_number` function within `utils.py` was replaced with the up-to-date version (defining encounters up to E26) for consistency, although it is no longer used by the fixed `get_perihelion_time`.
- The visual asymmetry observed in fixed-time-window plots using the degree axis was noted, and the use of `degrees_from_perihelion_range` is recommended for consistent scientific comparison.
- All relevant tests now pass, and visual inspection confirms correct behavior.

**Key Lessons Learned:**
- Verify data sources and helper functions independently (`get_perihelion_time` was the root cause).
- Debug prints tracing variable values through loops are crucial.
- Understand the difference between calculating based on *closest time in data* versus *interpolating at exact time*.
- A fixed *time* window does not guarantee a symmetrical *degree* window around perihelion; use explicit range options for consistency.

**Goal:** Implement `use_degrees_from_perihelion` as a new, mutually exclusive x-axis mode in `multiplot`, behaving identically to `x_axis_carrington_lon` in terms of axis formatting (ticks, labels), but with the plotted longitude values shifted so 0° corresponds to the longitude at the encounter's perihelion time. (Achieved)

**Verification:**
*   **[✅ Passing (Automated)]** Use `tests/multiplot_tests/text_x_axis_positional_types.py`.
*   **[✅ Verified (Debugging)]** Debugged and fixed the perihelion longitude calculation offset issue in `plotbot/utils.py`.
*   **[✅ Verified (Previous Fix)]** Standard positional axis issues were resolved by the fix in `XAxisPositionalDataMapper`.
*   **[✅ Verified (Visual)]** Visually verified plots using `Examples_Multiplot.ipynb`.
